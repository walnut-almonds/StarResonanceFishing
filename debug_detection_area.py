...
def main():
    ...
    # 2. 魚追踪區域
    fish_splash_region = config.get('detection', {}).get('fish_splash', {}).get('region', {})
    center_threshold = config.get('fishing', {}).get('fish_tracking', {}).get('center_threshold', 0.1)
    if fish_splash_region:
        ...
        center_x = window.width // 2 - 30  # 視窗中心X座標
    ...
    # 3. 拉力計檢測區域
    tension_bar_region = config.get('detection', {}).get('tension_bar', {}).get('region', {})
    ...
    # 4. 紅色張力檢測區域
    red_tension_region = config.get('detection', {}).get('red_tension', {}).get('region', {})
    ...
    # 6. 魚竿耐久度檢測區域
    rod_durability_region = config.get('detection', {}).get('rod_durability', {}).get('region', {})
