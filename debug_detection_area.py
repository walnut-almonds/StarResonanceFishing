...
def main():
    ...
    if bite_region:
        regions.append(draw_region_on_screen(
            window, bite_region,
            (0, 0, 255), "bite_region", 0
        ))

    # 2. 魚追踪區域
    fish_splash_region = config.get('detection', {}).get(
        'fish_splash', {}).get('region', {})
    center_threshold = config.get('fishing', {}).get(
        'fish_tracking', {}).get('center_threshold', 0.1)
    if fish_splash_region:
        regions.append(draw_region_on_screen(
            window, fish_splash_region,
            (0, 255, 0), "fish_splash_region", 30
        ))
        ...
        if left_boundary >= splash_x and left_boundary <= splash_x + splash_w:
            # 繪製左邊界線（相對於截圖座標）
            cv2.line(img, (left_boundary, splash_y),
                     (left_boundary, splash_y + splash_h), (255, 255, 0), 2)
        if right_boundary >= splash_x and right_boundary <= splash_x + splash_w:
            # 繪製右邊界線
            cv2.line(img, (right_boundary, splash_y),
                     (right_boundary, splash_y + splash_h), (255, 255, 0), 2)

        # 繪製中心線
        cv2.line(img, (center_x, splash_y),
                 (center_x, splash_y + splash_h), (255, 128, 0), 2)

        # 添加標註
        cv2.putText(img, "Center", (center_x + 5, splash_y + 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 128, 0), 2)
        cv2.putText(img, f"Threshold: {center_threshold:.1%}",
                    (splash_x, splash_y + splash_h + 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)

    # 3. 拉力計檢測區域
    tension_bar_region = config.get('detection', {}).get(
        'tension_bar', {}).get('region', {})
    if tension_bar_region:
        regions.append(draw_region_on_screen(
            window, tension_bar_region,
            (0, 255, 255), "tension_bar_region", 60
        ))

    # 4. 紅色張力檢測區域
    red_tension_region = config.get('detection', {}).get(
        'red_tension', {}).get('region', {})
    if red_tension_region:
        regions.append(draw_region_on_screen(
            window, red_tension_region,
            (0, 128, 255), "red_tension_region", 90
        ))

    # 5. "再來一次"按鈕檢測區域
    retry_button_region = config.get('detection', {}).get(
        'retry_button', {}).get('region', {})
    if retry_button_region:
        regions.append(draw_region_on_screen(
            window, retry_button_region,
            (255, 0, 255), "retry_button_region", 120
        ))

    # 6. 魚竿耐久度檢測區域
    rod_durability_region = config.get('detection', {}).get(
        'rod_durability', {}).get('region', {})
    if rod_durability_region:
        regions.append(draw_region_on_screen(
            window, rod_durability_region,
            (128, 0, 255), "rod_durability_region", 150
        ))
    ...
    if config.get('fishing', {}).get('cast_type') == 'click':
        ...
        if cast_pos:
            ...
            cv2.putText(img, "抛竿點擊位置", (click_x + 15, click_y),
    ...
    if first_click_pos:
        ...
        cv2.putText(img, "Rod Replace 1st Click", (first_x + 20, first_y - 10),
    ...
    if second_click_pos:
        ...
        cv2.putText(img, "Rod Replace 2nd Click", (second_x + 20, second_y - 10),
    ...
    if first_click_pos and second_click_pos:
        ...
        second_x = int(window.width * second_click_pos['x'])
        second_y = int(window.height * second_click_pos['y'])
        cv2.arrowedLine(img, (first_x, first_y), (second_x, second_y),
    ...
    for region_info in regions:
        ...
        cv2.rectangle(img, (x, y - 30 - offset_y),
                      (x + label_size[0] + 10, y - 5 - offset_y), color, -1)

        # 繪製標籤文字
        cv2.putText(img, label, (x + 5, y - 10 - offset_y),
